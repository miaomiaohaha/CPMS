...
$(document).ready(function(){
	...
	$("#items").datagrid({
		url:"${pageContext.request.contextPath }/personnal/train/findtrain",
		columns:[[
		          {field:"traino",title:"培训编号",width:100,hidden:true},       
		          {field:"trainame",title:"培训名称",width:100,
		        	  editor:{type:'text',options:{required:true}}}, 
		          {field:"traincontain",title:"培训内容",width:100,
		        	  editor:{type:'text',options:{required:true}}}, 
		          {field:"trainobj",title:"培训对象",width:100,
		        	  editor:{type:'text',options:{required:true}}}, 
		          {field:"traindatefrom",title:"开始时间",width:100,
		        	  editor:{type:'datebox',options:{required:true}}},
		          {field:"traindateto",title:"结束时间",width:100,
		        	  editor:{type:'datebox',options:{required:true}}},     
		          {field:"trainer",title:"培训讲师",width:100,
		        	  editor:{type:'text',options:{required:true}}},     
		          {field:"trainplace",title:"培训地点",width:100,
		        	  editor:{type:'text',options:{required:true}}},     
		          {field:"trainfee",title:"培训费用",width:100,
							 ...
							 handler: function(){
								 ...
								 {iconCls: 'icon-remove',
									  ...
									  handler: function(){
										  if("0"== limited){
											  var row = $("#items").datagrid("getSelected");
											  var traino = row.traino;
											  $.messager.confirm("提示", "您确定要删除这些数据吗？", function (res) {//提示是否删除
						                            if (res) {
						                            	$.ajax({
						                					url: "${pageContext.request.contextPath}/personnal/train/remove",
						                					data:{'traino':traino},
						                					type:"post",
				    	  ...
				    	  onLoadSuccess:function(data){
		    				...
		    				onAfterEdit: function(rowIndex, rowData, changes){
		    					...
		    					if(action == 1)
		    						...
		    						url = "add";
		    					$.ajax({
		    						url: "${pageContext.request.contextPath}/personnal/train/" + url,
		    						data: rowData,
		    						type:"POST",
		    						dataType:"json",
		    						success:function(data){
